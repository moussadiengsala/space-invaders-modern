...
export class Enemies extends Character {
  constructor(ID, size, health, strength, position, texture, spaceFire, directionBullets, gameBoard, direction) {
  ...
  move(numPoints, amplitude, frequency, enemies) {
    ...
    const x = (100 / numPoints) * this.directionX; // Spread the points evenly on the x-axis
    const y = amplitude * Math.sin((frequency * Math.PI) / numPoints) * this.directionY;
  ...
  adjustInCollision = throttle((enemies, newX, newY) => {
    for (let enemy of enemies) {
      if (enemy !== this) {
          let distance = Math.sqrt(Math.pow(newX - enemy.position.x, 2) + Math.pow(newY - enemy.position.y, 2));
          if (distance < 10) { // Adjust this value according to your needs
              ...
              // Apply the adjustments to avoid overlay
              newX += adjustX;
              newY += adjustY;
              break;
          }
